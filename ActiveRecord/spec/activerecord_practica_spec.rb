...
describe 'Practica ActiveRecord' do
  ...
  describe 'para encontrar' do
    ...
    describe 'using .where:' do
      ...
      specify 'con un correo electrónico válido (la dirección de correo electrónico contiene "@") (busca el operador SQL LIKE)' do
        check Customer.with_valid_email, [1,2,4,5,7,8,10,11,12,13,14,15,17,18,19,20,23,26,29,30]
      end
      specify 'con emails .org' do
        check Customer.with_dot_org_email, [5,7,8,12,23,26,29]
      end
      specify 'con correo electronico no valido pero que no este en blanco (no contiene "@")' do
        check Customer.with_invalid_email, [3,6,9,16,22,25,27,28]
      end
      specify 'con correo electronico en blanco' do
        check  Customer.with_blank_email, [21,24]
      end
      specify 'nacido antes del 1 Enero 1980' do
        check Customer.born_before_1980, [3,8,9,11,15,16,17,19,20,24,25,27]
      end
      specify 'con correo electronico valido Y nacidos antes del 1/1/1980' do
        check Customer.with_valid_email_and_born_before_1980, [8,11,15,17,19,20]
      end
      specify 'con apellidos que comienzan con "B", ordenados por fecha de nacimiento' do
        expect(Customer.last_names_starting_with_b.map(&:id)).to eq( [25,23,4,28,18,21,29,1] )
      end
    end
    describe 'sin necesidad de .where' do
      specify '20 clientes mas jovenes, en cualquier orden (busca ActiveRecord "order" y "limit")' do
  ...
  describe 'para actualizar' do
    ...
    specify 'la fecha de nacimiento de Gussie Murray hasta el 8 de febrero de 2004 (busca `Time.parse`)'do
      Customer.update_gussie_murray_birthdate
      expect(Customer.find_by(:first => 'Gussie').birthdate.to_date).to eq(Date.new 2004,2,8)
    end
    specify 'Todos los correos electronicos no validos deben estar en blanco.' do
    ...
    specify 'base de datos eliminando al cliente Meggie Herman' do
      Customer.delete_meggie_herman
      expect(Customer.find_by(:first => 'Meggie', :last => 'Herman')).to be_nil
    end
    specify 'base de datos eliminando todos los clientes nacidos el 31 de diciembre de 1977' do
